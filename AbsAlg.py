...
class logic(expression): #Logic expressions should be all which evaluate to True and False.
	...
	"""
	def prove(self, assumptions = set()):
		proving.add(self)
	...
	def prove(self, assumptions = set()): #Note:  If prove returns True, then we know the statement is True.  If it returns False, that is not definitive.
		if self in assumptions:
			return '[' + self.id + ':  Assumed]' #Want an output to recover method:  ['Proved Statement':  [Output of statement which proved it]]
		if Not(self) in assumptions:
			return False
		assumptions.add(Not(self))
		if self.key == 'not':
			out = self.args[0].disprove(assumptions)
		elif self.key == 'and':
			out = []
			for arg in self.args:
				res = arg.prove(assumptions)
				if not res
					out = False
					break
				out.append(res)
			...
			for arg in self.args:
				...
				if out:
					break
		elif self.key == 'implies':
			assumptions.add(self.args[0])
			out = self.args[1].prove(assumptions)
		if out:
			out = '[' + self.id + ':  ' + out + ']'
			assumptions.remove(Not(self))
			assumptions.add(self)
		return out
	
	forms = {'and','or','not','implies','equals','True','False','var','function'}
