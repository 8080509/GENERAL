...
class Matrix:
	...
	def matMult(self, mat):
		if (self.n == mat.m):
			return Matrix.matFromCell(lambda i,j: sum([self.content[i][k]*mat.content[k][j] for k in range(self.n)], zero), self.m, mat.n)
		else:
	...
	def det(self):
		if self.m == self.n:
			if self.n == 1:
				...
				return sum([((-one)**(k))*self.content[0][k]*(Matrix.matFromCell(lambda i,j: self.content[i+1][j+Utils.boolNum(j >= k)], self.m - 1, self.n - 1).det()) for k in range(self.n)], zero)
		else:
